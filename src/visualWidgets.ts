 ...
 */
export function calculateWidgetPosition(node: LGraphNode, inputIndex: number): WidgetPosition {
  return {
    x: NODE_CONSTANTS.MARGIN_X,
    y: NODE_CONSTANTS.HEADER_HEIGHT + 5 + (inputIndex * NODE_CONSTANTS.INPUT_HEIGHT),
    width: node.size[0]
  };
}
...
export function addAlignedWidget(
  ...
  if (config.type === 'file') {
    ...
    widget = node.addWidget(
      ...
      defaultValue,
      (value) => {
        const processedValue = config.type === 'number' ? 
 ...
 */
export function calculateNodeSize(inputCount: number, outputCount: number): [number, number] {
...
export function customizeNodeAppearance(node: LGraphNode) {
  ...
  node.getConnectionPos = function(is_input: boolean, slot: number | string, out?: Vector2): Vector2 {
